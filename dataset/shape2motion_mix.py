...
class Dataset(Dataset):
    def __init__(self, cfg, mode) -> None:
        ...
        self.num_catgory = len(cfg["dataset"]["category_" + self.mode])
        self.data_root = [join(
            cfg["root"],
            cfg["dataset"]["data_root"],
            cfg["dataset"]["category_" + self.mode][i]
            ) for i in range(self.num_catgory)]
        self.n_chunk_dict = {
        ...
        for i in range(self.num_catgory):
            ...
            with open(join(self.data_root[i], split_fn)) as f:
            ...
            for dp in split_data:
                ...
                sub_dir = os.path.join(self.data_root[i], obj_id)
                    ...
                    if t + self.set_size <= T:
                        ...
                        if self.input_type == "pcl":  # spare point-cloud input
                            ...
                            for view_id in range(num_views):
                                category_list.append({**meta, "view": view_id})
                    else:
                        break
                    if self.one_per_instance:
                        break
            self.meta_list += category_list
    ...
    def __getitem__(self, index: int):
        ...
        if self.augmentation and self.mode == "train":
            rotation = math.pi * np.random.choice([0, 0.5, 1, 1.5], size=1)

            if meta_info["category"] in ["laptop", "stapler"]:
                choice = np.random.choice([0,1,2], size=1)
                ret["axis_gt"] = np.array([[-1,0,0]], dtype=np.float)
                if choice == 0:
                    pass
                if choice == 1:
                    ret["axis_gt"] = -ret["axis_gt"][:, [1,0,2]]
            ...
            if meta_info["category"] in ['door',]:
                choice = np.random.choice([0,1], size=1)
                ret["axis_gt"] = np.array([[0,1,0]], dtype=np.float)
                if choice == 0:
                    pass
                if choice == 1:
                    ret["axis_gt"] = ret["axis_gt"][:, [0,2,1]]
                    for i in ["inputs", "points", "pointcloud"]:
                        ret[i] = ret[i][:,:,[0,2,1]]
                        ret[i][:,:,1] *= -1
            #rotation
            for i in ["inputs", "points", "pointcloud"]:
                ret[i][:,:,0], ret[i][:,:,2] = ret[i][:,:,0] * np.cos(rotation) - ret[i][:,:,2] * np.sin(rotation), ret[i][:,:,0] * np.sin(rotation) + ret[i][:,:,2] * np.cos(rotation)
            ret["axis_gt"][:,0], ret["axis_gt"][:,2] = ret["axis_gt"][:,0] * np.cos(rotation) - ret["axis_gt"][:,2] * np.sin(rotation), ret["axis_gt"][:,0] * np.sin(rotation) + ret["axis_gt"][:,2] * np.cos(rotation)
