...
class Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        meta_info['category'] = list(meta_info['dir'].split('/'))[-2]
        ...
        if meta_info['category'] in ['door']:
            ret["axis_o"] = np.array([[0,1,0]], dtype=np.float)
            ret["theta_canon"] = np.array([1/3 * math.pi], dtype=np.float)
            ret["theta_range"] = np.array([[-1/6*math.pi, 1/3*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art0060'

        elif meta_info['category'] in ['washing_machine']:
            ret["axis_o"] = np.array([[0,-1,0]], dtype=np.float)
            ret["theta_canon"] = np.array([1/3 * math.pi], dtype=np.float)
            ret["theta_range"] = np.array([[-1/6*math.pi, 1/3*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art0060'

        elif meta_info['category'] in ['stapler']:
            ret["axis_o"] = np.array([[-1,0,0]], dtype=np.float)
            ret["theta_canon"] = np.array([1/3 * math.pi], dtype=np.float)
            ret["theta_range"] = np.array([[-1/6*math.pi, 1/3*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art0060'

        elif meta_info['category'] in ['laptop']:
            ret["axis_o"] = np.array([[-1,0,0]], dtype=np.float)
            ret["theta_canon"] = np.array([0], dtype=np.float)
            ret["theta_range"] = np.array([[-1/6*math.pi, 1/2*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art0000'

        elif meta_info['category'] in ['oven']:
            ret["axis_o"] = np.array([[1,0,0]], dtype=np.float)
            ret["theta_canon"] = np.array([1/2 * math.pi], dtype=np.float)
            ret["theta_range"] = np.array([[0*math.pi, 1/2*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art0090'

        elif meta_info['category'] in ['eyeglasses']:
            ret["axis_o"] = np.array([[0,1,0], [0,-1,0]], dtype=np.float)
            ret["theta_canon"] = np.array([0*math.pi, 20/180*math.pi], dtype=np.float)
            ret["theta_range"] = np.array([[-50/180*math.pi, 0*math.pi],[-30/180*math.pi, 20/180*math.pi]], dtype=np.float)
            canonical_pc_file = meta_info["files"][0].split('art')[0] + 'art00000020'
        '''
        _occ_data = self.load(
                    join(base_root, "implicit"),
                    canonical_pc_file,
        ...
        # transform the canonical view occ queries
        _q = (object_T[:3, :3] @ _q.T + object_T[:3, 3:4]).T
        ret["points_canon"] = _q.astype(float)
        ret["points_canon.occ"] = _o.astype(float)
        ...
        if self.mode == "train":
            ...
            choice_inputs = np.array([i for i in range(self.n_inputs)])
        ret["pointcloud_canon"] = pc[choice_inputs]
        '''
        # fake time for LPDC
        ret["fake_time"] = np.linspace(start=0.0, stop=1.0, num=self.set_size)

        if self.rotate:
            rand_vec = torch.rand(1, 3) - 0.5
            rand_vec[:,0] = 0
            rand_vec[:,2] = 0
            rand_vec = ExpSO3(2 * math.pi * rand_vec).unsqueeze(1).numpy()#1,1,3,3
            for i in ["inputs"]:
                ret[i] = (rand_vec @ ret[i][..., np.newaxis]).squeeze(-1)
            for i in ["axis_o"]:
                ret[i] = (rand_vec[:,0] @ ret[i][..., np.newaxis]).squeeze(-1)
