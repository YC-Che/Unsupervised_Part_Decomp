    ...
    def run(self):
        ...
        while self.current_epoch <= self.total_epoch:
            ...
            if self.current_epoch % 50 == 1:
                weight_dir = os.path.join(self.cfg["root"], "log", self.cfg["logging"]["log_dir"], "weight")
                if not os.path.exists(weight_dir):
                    os.makedirs(weight_dir)
                torch.save(batch['data']["viz_query"], weight_dir+'/query_'+str(self.current_epoch)+'.pt')
                torch.save(batch['data']["viz_weights"], weight_dir+'/weights_'+str(self.current_epoch)+'.pt')
                torch.save(batch['data']["viz_trans"], weight_dir+'/c_trans_'+str(self.current_epoch)+'.pt')
                torch.save(batch['data']["viz_axis"], weight_dir+'/c_axis_'+str(self.current_epoch)+'.pt')
                torch.save(batch['data']["viz_length"], weight_dir+'/c_length_'+str(self.current_epoch)+'.pt')
            self.current_epoch += 1
        self.logger.end_log()
        return
